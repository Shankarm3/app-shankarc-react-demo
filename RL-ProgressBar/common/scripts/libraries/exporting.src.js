...
Highcharts.post = function (url, data) {
	...
	form = createElement('form', {
		...
		encoding: 'multipart/form-data' // IE8 bug
	}, {
		display: NONE
	}, doc.body);
...
extend(Chart.prototype, {
	 ...
	 */
	exportChart: function (options, chartOptions, z) {
		
		options = options || {};
		
		var chart = this;
		var mergedChartData = merge(
            { chart: { borderRadius: 0 } }, // docs: defaults to 0 for exported charts
            chart.options.exporting.chartOptions, // docs
            chartOptions,
            {
                exporting: {
                    sourceWidth: chart.options.exporting.sourceWidth, // docs
                    sourceHeight: chart.options.exporting.sourceHeight // docs
                }
            }
        );
		var svg = chart.getSVG(mergedChartData);
		...
		var l = {
		    ...
		    divisions: options.metadata.divArr
		};
	    // do the post
	    !z || z == {} ? l.svg = svg : false;
	    Highcharts.post(options.url, z && z != {}?$.extend(!0,l,z):l);
	},
