		...
		switch (eltype)
		{
			...
			case "custom" :
				...
				try {
					...
					if (e=="e1") { $.jgrid.info_dialog($.jgrid.errors.errcap,$.jgrid.edit.msg.nodefined, $.jgrid.edit.bClose);}
					if (e=="e2") { $.jgrid.info_dialog($.jgrid.errors.errcap,$.jgrid.edit.msg.novalue,$.jgrid.edit.bClose);}
	...
	checkValues : function(val, valref,g, customobject, nam) {
		...
		if(edtrul) {
			...
			if(edtrul.required === true) {
				if( $.jgrid.isEmpty(val) )  { return [false,$.jgrid.edit.msg.required,""]; }
			}
			// force required
			...
			if(typeof edtrul.minValue != 'undefined' && !isNaN(edtrul.minValue)) {
				if (parseFloat(val) < parseFloat(edtrul.minValue) ) { return [false,$.jgrid.edit.msg.minValue+" "+edtrul.minValue,""];}
			}
			if(typeof edtrul.maxValue != 'undefined' && !isNaN(edtrul.maxValue)) {
				if (parseFloat(val) > parseFloat(edtrul.maxValue) ) { return [false,$.jgrid.edit.msg.maxValue+" "+edtrul.maxValue,""];}
			}
			var filter;
			if(edtrul.email === true) {
				...
				// taken from $ Validate plugin
					...
					if(!filter.test(val)) {return [false,$.jgrid.edit.msg.email,""];}
				}
			}
			if(edtrul.integer === true) {
				if( !(rqfield === false && $.jgrid.isEmpty(val)) ) {
					if(isNaN(val)) { return [false,$.jgrid.edit.msg.integer,""]; }
					if ((val % 1 !== 0) || (val.indexOf('.') != -1)) { return [false,$.jgrid.edit.msg.integer,""];}
				}
			}
			if(edtrul.date === true) {
				if( !(rqfield === false && $.jgrid.isEmpty(val)) ) {
					...
					if(!$.jgrid.checkDate (dft, val)) { return [false,$.jgrid.edit.msg.date+" - "+dft,""]; }
				}
			}
			if(edtrul.time === true) {
				if( !(rqfield === false && $.jgrid.isEmpty(val)) ) {
					if(!$.jgrid.checkTime (val)) { return [false,$.jgrid.edit.msg.date+" - hh:mm (am/pm)",""]; }
				}
			}
			if(edtrul.url === true) {
				if( !(rqfield === false && $.jgrid.isEmpty(val)) ) {
					filter = /^(((https?)|(ftp)):\/\/([\-\w]+\.)+\w{2,3}(\/[%\-\w]+(\.\w{2,})?)*(([\w\-\.\?\\\/+@&#;`~=%!]*)(\.\w{2,})?)*\/?)/i;
					if(!filter.test(val)) {return [false,$.jgrid.edit.msg.url,""];}
				}
			}
						...
						$.each($t.p.colModel, function(){
							if(this.name === nm && this.editoptions && $.isFunction(this.editoptions.custom_value)) {
								try {
									...
									if (e==="e1") {$.jgrid.info_dialog(jQuery.jgrid.errors.errcap,$.jgrid.edit.msg.novalue,jQuery.jgrid.edit.bClose);}
						...
						switch (cm[i].edittype) {
							...
							case 'custom' :
								try {
									...
									if (e=="e1") {$.jgrid.info_dialog(jQuery.jgrid.errors.errcap,$.jgrid.edit.msg.nodefined,jQuery.jgrid.edit.bClose);}
									else {$.jgrid.info_dialog(jQuery.jgrid.errors.errcap,e.message,jQuery.jgrid.edit.bClose);}
								}
								break;
						}
				...
				switch (cm.edittype) {
					...
					case 'custom' :
						try {
							...
							if (e=="e1") { $.jgrid.info_dialog(jQuery.jgrid.errors.errcap,$.jgrid.edit.msg.nodefined,jQuery.jgrid.edit.bClose); }
							if (e=="e2") { $.jgrid.info_dialog(jQuery.jgrid.errors.errcap,$.jgrid.edit.msg.novalue,jQuery.jgrid.edit.bClose); }
							else {$.jgrid.info_dialog(jQuery.jgrid.errors.errcap,e.message,jQuery.jgrid.edit.bClose); }
						}
						break;
				}
