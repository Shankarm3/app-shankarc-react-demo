...
//        });
            ...
            if (screens.performanceComparison.viewType == "chartView") {
            	...
            	$.when(
            		...
            		if (globalvars.performanceComparisonChart.data.length == 0) {
                    	...
                    	filterParameters.metricType='reviewRate'; // hard coded for physician as metric type is not there in filter
   ...
   loadListData: function (filterParameters) {
	   ...
	   $.when(
		   ...
		   globalvars["performanceComparisonList"] = data1;
            ...
            if( (globalvars.user.uType == globalvars.roles.executive || globalvars.user.uType == globalvars.roles.reportUser || globalvars.user.uType == globalvars.roles.supervisor || globalvars.user.uType==globalvars.roles.administrator) && globalvars.root.auditLevel=="2" ){
         	   screens.performanceComparison.downloadExcelURI = globalvars.root.performanceComparisonListViewExcelUri + "?period="+filterParameters.period + hospitalString + costCenterString + "&hitType=" + filterParameters.hitType;
         	   screens.performanceComparison.downloadPdfURI = globalvars.root.performanceComparisonListViewPdfUri + "?period="+filterParameters.period + hospitalString + costCenterString +"&hitType=" + filterParameters.hitType;
            }else{
         	   screens.performanceComparison.downloadExcelURI = globalvars.root.performanceComparisonListViewExcelUri + "?period="+filterParameters.period + hospitalString+ costCenterString;
                screens.performanceComparison.downloadPdfURI = globalvars.root.performanceComparisonListViewPdfUri + "?period="+filterParameters.period + hospitalString + costCenterString;
            }
    ...
    showListData: function () {
        ...
        gridPhysician.loadPerformanceComparisonGrid({
    ...
    showChartData: function (metricType) {
        ...
        for(var i=0;i<globalvars.performanceComparisonChart.data.length;i++){
        	parentTotalReviewRate = parentTotalReviewRate + globalvars.performanceComparisonChart.data[i].hitCount;
        ...
        for(var i=0;i<globalvars.performanceComparisonChart.data.length;i++){
        	...
        	if(metricType === "foundPerDay"){
        		...
        		currentObj.addtionalValue = currentObj.hitCount;
        		...
        		currentObj.origValue = currentObj.hitCount;
        		currentObj.addtionalValue = currentObj.hitValue;
        		if(parentTotalReviewRate!=0){
            		if((currentObj.hitCount!=0) && (currentObj.hitCount/parentTotalReviewRate<0.01)){
            			currentObj.hitCount = parentTotalReviewRate*0.01;
            		}
        		}        		
        	}
        }
        ...
        for(var i=0;i<globalvars.performanceComparisonChart.data.length;i++){
        	...
        	for(var j =0;j<currentObjHospitalData.length;j++){
        		childTotalReviewRate = childTotalReviewRate + currentObjHospitalData[j].hitCount;
        	...
        	for(var k =0;k<currentObjHospitalData.length;k++){
        		if(metricType === "foundPerDay"){
            		...
            		currentObjHospitalData[k].addtionalValue = currentObjHospitalData[k].hitCount;
            		...
            		currentObjHospitalData[k].origValue = currentObjHospitalData[k].hitCount;
            		currentObjHospitalData[k].addtionalValue = currentObjHospitalData[k].hitValue;
            		if(childTotalReviewRate!=0){
            			if((currentObjHospitalData[k].hitCount!=0) && (currentObjHospitalData[k].hitCount/childTotalReviewRate<0.01)){
                			currentObjHospitalData[k].hitCount = childTotalReviewRate*0.01;
                		}
            		}
            	}
        	}
        ...
        for(var i=0;i<globalvars.performanceComparisonChart.data.length;i++){
            ...
            obj.origValue = currentObj.origValue;
	   	    ...
	   	    for(var j =0;j<currentObjHospitalData.length;j++){
	      		totalChildrenReviewRate = totalChildrenReviewRate + currentObjHospitalData[j].hitCount;
	      		totalChildrenHitValue = totalChildrenHitValue + currentObjHospitalData[j].hitValue;
	      	}
   	        
   	        if(metricType === "reviewRate"){
   	        	parentReviewRate = currentObj.hitCount;
	        	 ...
	        	 if(metricType === "foundPerDay"){
	        		if(totalChildrenHitValue!=0){
	        		 ...
	        		 if(totalChildrenReviewRate!=0){
	        			 obj1.value = (currentObjHospitalData[j].hitCount/totalChildrenReviewRate)*parentReviewRate;
		        	 }
	        	 }
      ...
      function display(d) {
        ...
        /* write parent rectangle */
          ...
          .text(function(d){
        	  ...
        	  if(metricType === "foundPerDay"){
          		return d.name +" - $"+Math.round(d.addtionalValue)+" ("+chartsCurrencyFormatter(d.origValue)+") "+globalvars.localResourceMap.heat_map_chart_tooltip_text1+" " + d.variance + "%";
         	  }else if(metricType === "reviewRate"){
         			return d.name +" - $"+chartsCurrencyFormatter(d.addtionalValue)+" ("+Math.round(d.origValue)+") "+globalvars.localResourceMap.heat_map_chart_tooltip_text2+" " + d.variance + "%";
         	  }
             ...
             .text(function(d) {
            	if(metricType === "foundPerDay"){
            		return d.name +" - $"+chartsCurrencyFormatter(d.origValue)+" ("+Math.round(d.addtionalValue)+")";
           			...
           			return d.name +" - $"+chartsCurrencyFormatter(d.addtionalValue)+" ("+Math.round(d.origValue)+")";
           		}
        	...
        	if (screens.performanceComparison.viewType == "chartView" && globalvars.performanceComparisonChart.data.length == 0) {
          	...
          	if(screens.performanceComparison.viewType == "chartView"){
        		...
        		//export_svg = export_svg.replace(/<\/g><\/svg>/,'<g><defs><linearGradient id="Gradient2"><stop offset="20%" stop-color="#003A70"/><stop offset="40%" stop-color="#0067CC"/><stop  offset="70%" stop-color="#689FCF" /><stop offset="100%" stop-color="#CCCCCC" /></linearGradient></defs><text y="695" x="10"> Box Size = % Review Rate </text><rect x="10" y="700"  width="200" height="25" fill="url(#Gradient2)"/></g></g></svg>')
          		...
          		exportCharts({ filename: 'performanceComparison', sourceWidth: 0, sourceHeight: 0, scale: 1, metadata: { title: globalvars.localResourceMap.dashboard_screen_performance_comparison, filter: formatFiltersForChartExportPhysician } }, undefined, export_svg);
        		...
        		$("#export_options").show();
        	   	$("#download_pdf").off().on("click", function (event) {
                    ...
                    $.fileDownload(screens.performanceComparison.downloadPdfURI, {
              	...
              	$("#download_excel").off().on("click", function (event) {
                    ...
                    var $preparingFileModal = $("#preparing-file-modal");
                    $preparingFileModal.dialog({ modal: true });
                    $.fileDownload(screens.performanceComparison.downloadExcelURI, {
                        successCallback: function (url) {
                            $preparingFileModal.dialog('destroy');
                        },
                        failCallback: function (responseHtml, url) {
    ...
    populateTreeViewData:function(){
    	...
    	if(globalvars.performanceComparisonList.data)
    	{
         ...
         for (var i = 0; i < treeListData.length; i++) {
	    	  ...
	    	  treeListData[i].hitCount = Math.round(treeListData[i].hitCount);
