screens.physicianDashboard = {
    ...
    loadData: function(filterParameters){
        ...
        var tempFilterParameters4 = $.extend(true, {}, filterParameters);
        tempFilterParameters4.period = 4;
        ...
        $.when(
        		...
        		$.ajax({
        			type: 'GET',
		            url: globalvars.root.physicianTopProcedureCodeUri,
		            data: filterParameters,
		            traditional: true,
		            dataType: 'json'
        		})
        ...
        ).done(function(data1, data2, data3,data31, data4, data5,data6){
        	...
        	globalvars["summaryChartDataYearToDate"] = data3[0];
                globalvars["summaryChartDataLastWeek"] = data31[0];
        	...
        	//globalvars["rlImpact"] = data9[0];
        	screens.physicianDashboard.fillScreen(filterParameters);
        });
    ...
    fillScreen: function (filterParameters) {
        ...
        widgets.performanceSummaryChart.showData({
            current: {
                ...
                dollarsFoundPerDay: globalvars.summaryChartDataCurrentMonth.dollarsFoundPerDay
            },
            lastWeek: {
        ...
        function calculateCount(){
            if(globalvars.dashboardOverallTrendsChartData.yAxis){
             var reviewCountData = globalvars.dashboardOverallTrendsChartData.yAxis[3];
             var maxValue=0;
        ...
        $.each(globalvars.performanceComparisonDashboard.data, function (index) {
        	$(".performance_comparison_widget_chart_div_title").eq(index).text(this.costCenter);
		...
		$.each(globalvars.dashboardTopFiveChargesChartData.yAxis,function(i){
			var dataObj = { procedureDescription: this.procedureDescription,y: this.y};
		...
		if(globalvars.dashboardTopFiveChargesChartData.yAxis.length>0){
			if(globalvars.dashboardTopFiveChargesChartData.yAxis.length<5){
				for(var i = globalvars.dashboardTopFiveChargesChartData.yAxis.length;i<5;i++){
					...
					var dataObj = {procedureDescription:0,y:0};
					dataArray.push(dataObj);
        ...
        if (globalvars.dashboardTopFiveChargesChartData.yAxis.length != 0) {
            charts.topFiveProcedureBarChart.initialize({
    ...
    drawScreen: function () {
        ...
        var filterTemplate = getTemplate('common/templates/screens/physicianDashboard.html?100');
    ...
    bindFunctionality: function () {
        ...
        $('.top_five_view_more').eq(0).click(function () {
            transferFilters('physicianDashboard', 'performanceComparison',"");
        ...
        $('.top_five_view_more').eq(1).click(function () {
            transferFilters('physicianDashboard', 'facilityPerformance',"CHARGE");
            $("#sub_menu #facility").show();
            var subMenuItems = $("#sub_menu #facility li.submenu_item");
            if(subMenuItems.length>0){
            	$("#sub_menu #facility li.submenu_item").eq(subMenuItems.length-1).addClass('submenu_last_item');
        ...
        $('.top_five_view_more').eq(2).click(function () {
            transferFilters('physicianDashboard', 'facilityPerformance', "DEPT");
            $("#sub_menu #facility").show();
            var subMenuItems = $("#sub_menu #facility li.submenu_item");
            if(subMenuItems.length>0){
            	$("#sub_menu #facility li.submenu_item").eq(subMenuItems.length-1).addClass('submenu_last_item');
    ...
    bindExportFunctionality:function(){
        ...
        $(".export_div_print").click(function(){
        		...
        		if(globalvars.performanceComparisonDashboard.data.length == 0)
        		{
        		    ...
        		    if(chartContainer1!=undefined){
        		    	printPerformanceComparisonChart({
             		    	...
             		    	screenName :"physicianDashboardPerformanceChart"
             		    });
        		    }
        	...
        	}else if(chartType == 'Top Procedure codes'){
        ...
        $(".export_div_download").click(function(){          
          ...
          var chartType = $(this).parent().parent().parent().find("h3").text();
            ...
            console.log(divArr);
            d = log(a), e = $(m).highcharts().exportChart({ filename: 'physicianDashboard', sourceWidth: 0, sourceHeight: 0, scale: 1, metadata: { title: globalvars.localResourceMap.dashboard_screen_performance_comparison, filter: formatFiltersForChartExportPhysician, divArr: JSON.stringify(divArr) } }, undefined, a);
    ...
    drawTopFiveCharts: function () {
        var topFiveTemplate = getTemplate('common/templates/top_five_physician.html?100');

        $('#top_five_widget_wrapper').append($.nano(topFiveTemplate, {
            heading1: 'Top Procedure codes',
