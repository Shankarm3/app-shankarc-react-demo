screens.publishCodesList = {
	...
	drawScreen: function () {
		...
		getSYNC('common/templates/screens/publishCodesList.html', function (data) {
	...
	loadData: function(){
    	...
    	$.when(
    		...
    		globalvars["CodesSummary"] = data;
	   		if (gridPhysician.CodesSummaryGrid.$gridDiv.jqGrid)
		            gridPhysician.CodesSummaryGrid.$gridDiv.jqGrid('GridUnload');
			...
			gridPhysician.CodesSummaryGrid.initialize({
		          data: globalvars.CodesSummary,
		          gridDiv: "#codes_grid_table",
		          pagerDiv: "#prebill_grid_pager"
			});

			screens.publishCodesList.bindFunctionality();
