 ...
 * @author Nirmal Kumar
 */
@Path("/" + PhysicianAuditorAssignmentResource.URL_AUDITOR_ASSIGNMENT)
@Produces(MediaType.APPLICATION_JSON)
@RequiresAuthentication
public class PhysicianAuditorAssignmentResource {

    public static final String URL_AUDITOR_ASSIGNMENT = "physicianAuditorAssignment";

    protected static final Logger log = LoggerFactory.getLogger(PhysicianAuditorAssignmentResource.class);
    ...
    @Inject
    public PhysicianAuditorAssignmentResource(PhysicianAuditorAssignmentDao businessDao, UserDao userDao) {
    ...
    public List<AuditorAssignmentRepresentation> getAllAssignments() {
        ...
        List<PhysicianAuditorAssignmentResult> physicianBillData = businessDao.getAllAssignments();
        // List<PhysicianAuditorAssignmentResult> queuePhysicianData = businessDao.getAllAssignmentsQueue();
    ...
    public Response updateAssignments(List<AuditorAssignmentRepresentation> input) throws RevenueLeakageException {
        ...
        PhysicianAuditorAssignmentUtils util = new PhysicianAuditorAssignmentUtils(current);
        ...
        for (AuditorAssignmentRepresentation oneHospital : input) {
            ...
            if (n.PHYS) {
                log.debug("oneHospital.source,oneHospital.costCenter,oneHospital.hospitalId  " + oneHospital.source, oneHospital.costCenter, oneHospital.hospitalId + " no longer has any pre bill auditors. Unassigning all unreviewed pre bill accounts...");
                businessDao.unassignAccounts(oneHospital.source, oneHospital.costCenter, oneHospital.hospitalId);
            }
            ...
            for (Difference difference : result.differences) {
                if (difference.type == OperationType.DELETE) {
                    ...
                    businessDao.deleteAssignment(difference.userId, oneHospital.hospitalId, oneHospital.source, oneHospital.costCenter, AuthUtils.getLoggedUserName(), currentTime, (byte) 1);
                    ...
                    if (userHospitalMappingForDifferenceOld.containsKey(difference.userId)) {
                        ...
                        PhysicianResult pr = new PhysicianResult(oneHospital.source, oneHospital.costCenter, oneHospital.hospitalId);
                        newHospList.add(pr);
                    } else {
                        PhysicianResult pr = new PhysicianResult(oneHospital.source, oneHospital.costCenter, oneHospital.hospitalId);
                ...
                if (difference.type == OperationType.ADD) {
                    ...
                    record.setHospitalId(oneHospital.hospitalId);
        ...
        if (!userHospitalMappingForDifferenceNew.isEmpty()) {
            ...
            sendAuditorAssignmentEmail(false, userHospitalMappingForDifferenceNew, EmailConstants.PHYS_ASSIGNMENT_MESSAGE, EmailConstants.PHYS_NON_REAL_TIME_AUDITOR_ASSIGNMENT_NEW_USER_MESSAGE);
        }
...
//        }
    ...
    protected void sendAuditorAssignmentEmail(boolean flag, Map<String, HospitalData> map, String assigmentMessage1, String assigmentMessage2) throws RevenueLeakageException {
        for (Map.Entry<String, HospitalData> entry : map.entrySet()) {
            if (!entry.getValue().physicianHospitalList.isEmpty()) {
                ...
                if (!entry.getValue().physicianHospitalList.isEmpty()) {
                    hospParam = "<br/><TABLE style=\"border-collapse: collapse;border: 1px solid black;\"><thead><TH style=\"border: 1px solid black;padding: 5px;text-align: left;\"><b>Source</b></TH><TH style=\"border: 1px solid black;padding: 5px;text-align: left;\"><b>Cost Center</b></TH><TH style=\"border: 1px solid black;padding: 5px;text-align: left;\"><b>Facility</b></TH></thead>";
                }
                for (PhysicianResult physicianResult : new HashSet<PhysicianResult>(map.get(entry.getKey()).physicianHospitalList)) {
                    hospParam = hospParam + "<tr><td style=\"border: 1px solid black;padding: 5px;text-align: left;\" >" + physicianResult.source + "</td><td style=\"border: 1px solid black;padding: 5px;text-align: left;\" > " + physicianResult.costCenter + "</td><td style=\"border: 1px solid black;padding: 5px;text-align: left;\" >" + physicianResult.hospitalId + "</td></tr>";
                }
                if (!entry.getValue().physicianHospitalList.isEmpty()) {
        ...
        for (PhysicianResult physicianResult : map.keySet()) {
            if (!map.get(physicianResult).physicianHospitalListMap.isEmpty()) {
                ...
                for (AccountList accountList : accountLists) {
                    if (accountList.source.equalsIgnoreCase(physicianResult.source) && accountList.costCenter.equalsIgnoreCase(physicianResult.costCenter) && accountList.hospitalId.equalsIgnoreCase(physicianResult.hospitalId)) {
    ...
    private void overrideAccount(List<AuditorAssignmentRepresentation> current) {
        for (AuditorAssignmentRepresentation record : current) {
            if (record.auditorData.size() > 1) {
                List<AccountList> accountList = businessDao.getAllOverlapsAccount(record.hospitalId, record.costCenter, record.source);
                for (AccountList account : accountList) {
                    businessDao.updatePredictionOverlaps(record.hospitalId, record.costCenter, record.source, (String) record.auditorData.toArray()[0], account.accountId);
                }
            }
        }
    }
    ...
    public static class HospitalData {
        ...
        public List<PhysicianResult> physicianHospitalList = new ArrayList<PhysicianResult>(0);
    ...
    protected boolean isInputObjectValid(List<AuditorAssignmentRepresentation> input) {
        ...
        for (AuditorAssignmentRepresentation one : input) {
            ...
            if (one.hospitalId == null || one.hospitalId.trim().isEmpty()) {
                log.error("HospitalId is null or empty string!");
     ...
     * @return List<AuditorAssignmentRepresentation>
     */
    protected List<AuditorAssignmentRepresentation> mergeResults(List<PhysicianAuditorAssignmentResult> physicainData) {
        ...
        for (PhysicianAuditorAssignmentResult one : physicainData) {
            PhysicianResult physicianResult = new PhysicianResult(one.source, one.costCenter, one.hospitalId);
            ...
            if (oneRecord == null) {
                ...
                newRecord.hospitalId = one.hospitalId;
                newRecord.costCenter = one.costCenter;
                newRecord.hospitalName = one.hospitalName;
                newRecord.shortName = one.shortName;
     ...
     * @return List<AuditorAssignmentRepresentation>
     */
    protected List<PhysicianAuditorAssignmentResult> chageOriginalStructure(List<AuditorAssignmentRepresentation> physicainData) {
        Set<PhysicianAuditorAssignmentResult> list = new HashSet<PhysicianAuditorAssignmentResult>(0);
        for (AuditorAssignmentRepresentation oneRecord : physicainData) {
            boolean flag = true;
            for (String auditorId : oneRecord.auditorData) {
                PhysicianAuditorAssignmentResult one = new PhysicianAuditorAssignmentResult();
                one.source = oneRecord.source;
                one.costCenter = oneRecord.costCenter;
                one.hospitalId = oneRecord.costCenter;
        ...
        return new ArrayList<PhysicianAuditorAssignmentResult>(list);
    }
    ...
    public static class AuditorAssignmentRepresentation {
        ...
        public Set<String> queueAuditors = new HashSet<String>(0);
     ...
     * @author Nirmal Kumar
     */
    public static class PhysicianResult implements Comparable<PhysicianResult> {

        public String source;
        public String costCenter;
        public String hospitalId;

        public PhysicianResult(String source, String costCenter, String hospitalId) {
            this.source = source;
        ...
        public int hashCode() {
            int hash = 5;
            String concateKey = this.source.toLowerCase() + "-" + this.costCenter.toLowerCase() + "-" + this.hospitalId.toLowerCase();
        ...
        public boolean equals(Object obj) {
            ...
            String concateKey = this.source.toLowerCase() + "-" + this.costCenter.toLowerCase() + "-" + this.hospitalId.toLowerCase();
            String concateKey1 = other.source.toLowerCase() + "-" + other.costCenter.toLowerCase() + "-" + other.hospitalId.toLowerCase();;
            return concateKey.equals(concateKey1);
        }

        @Override
        public int compareTo(PhysicianResult o) {
            final PhysicianResult other = new PhysicianResult(this.source, this.costCenter, this.hospitalId);
    ...
    public void runRules(Map<PhysicianResult, AuditorDataAssignment> map, Map<String, HospitalData> userHospitalMappingForDifferenceNew, Map<String, HospitalData> userHospitalMappingForDifferenceOld, Map<String, HospitalData> userHospitalMappingForExisting, long currentTime) {
        ...
        for (AuditorAssignmentRuleResource.RuleResult ruleResult : list) {
            ...
            if (size != 0) {
                ...
                for (PhysicianResult physicianResult : map.keySet()) {
                    if (!map.get(physicianResult).physicianHospitalListMap.isEmpty()) {
                        accountList.addAll(auditorAssignmentRuleDao.getActualAccountFlag(ruleResult.ruleId, physicianResult.hospitalId, physicianResult.source, physicianResult.costCenter, predDatePre));
        ...
        for (Auditor auditor : deleteAuditorList) {
            businessDao.deleteAssignment(auditor.userId, auditor.hospitalId, auditor.source, auditor.costCenter, AuthUtils.getLoggedUserName(), currentTime, (byte) 0);
    ...
    public void updateAssignmentByQueue(Map<String, HospitalData> userMappingForHospital, Auditor auditor) {
        log.debug("Processing new user...");
        PhysicianResult result = new PhysicianResult(auditor.source, auditor.costCenter, auditor.hospitalId);
