...
import com.operasolutions.rl.service.physician.auditor.assignment.PhysicianAuditorAssignmentResource.AuditorAssignmentRepresentation;
 ...
 * @author Nirmal Kumar
 */
public class PhysicianAuditorAssignmentUtils {

    protected static final Logger log = LoggerFactory.getLogger(PhysicianAuditorAssignmentUtils.class);
     ...
     * @param current
     */
    public PhysicianAuditorAssignmentUtils(List<AuditorAssignmentRepresentation> current) {
        if (current == null) {
        ...
        for (AuditorAssignmentRepresentation one : current) {
            oldRecords.put(one.source + one.costCenter + one.hospitalId, one);
        }
    }
    ...
    public NoReassign getIfReassignedNoAuditor(AuditorAssignmentRepresentation newRecord) {
        ...
        if (oldRecords.get(newRecord.source + newRecord.costCenter + newRecord.hospitalId).auditorData.size() != 0 && newRecord.auditorData.size() == 0) {
            n.PHYS = true;
        }
    ...
    public DiffResult getDifferences(AuditorAssignmentRepresentation newRecord) {
        ...
        log.debug("Finding differences for hospitalId = " + newRecord.source + newRecord.costCenter + newRecord.hospitalId);
        DiffResult result = new DiffResult();

        AuditorAssignmentRepresentation oldRecord = oldRecords.get(newRecord.source + newRecord.costCenter + newRecord.hospitalId);
        if (oldRecord != null) {
