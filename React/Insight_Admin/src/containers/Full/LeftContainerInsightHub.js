...
export default class LeftContainerInsight extends React.Component {
	...
	render(){
		...
		var listItems = this.state.data.map((item, index)=>{
			...
			return <ListGroupItem data={item["id"]} key={item["id"]}><Link to='/insight_detail' data={item["id"]} onClick={this.handleClick}>{item["insight_text"]}<br/><small>{item["time_period"]}</small></Link></ListGroupItem>
		})
		return(
			<div>
