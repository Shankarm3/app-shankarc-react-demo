...
export default class LeftContainerInsightDetail extends React.Component {
	...
	render(){
		...
		var listItems = this.state.data.map((item, index)=>{
			...
			return <ListGroupItem data={item["id"]} key={item["id"]} className={activeClass}><Link to='/insight_detail' data={item["id"]} onClick={this.handleClick}>{item["insight_text"]}<br/> <span className="pull-right time-period-text"><small>{item["time_period"]}</small></span></Link></ListGroupItem>
		})
		return(
			<div id="insight-detail-wrapper">
