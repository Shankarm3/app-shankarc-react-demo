...
export default class RightContainerSearch extends React.Component {
	...
	render(){
			...
			var listItems = relatedSearches.map((item, index)=>{
				...
				return <ListGroupItem className={activeClass} key={item["id"]+""+index}><Link to='/search_page' data={item["id"]} data-key={item.id+"::"+item.label} onClick={this.handleClick}>{item["label"]}</Link></ListGroupItem>
			})
			return(
